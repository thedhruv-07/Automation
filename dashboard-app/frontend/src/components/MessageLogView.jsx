  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          <h2 className="text-2xl font-bold text-ink-primary">Message Log</h2>
          <p className="text-ink-secondary text-sm mt-1">
            Real record of WhatsApp renewal alerts actually sent (from sent_log.json).
          </p>
        </div>
      ...
      {loadError && (
        ...
        >
          Could not load message log: {loadError}.
        </div>
      )}
      ...
      <div className="bg-surface rounded-xl border border-line overflow-hidden">
        <div className="px-4 py-3 border-b border-line flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={() => downloadMessageLogCsv(rows)}
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-surface-page text-xs uppercase tracking-wide text-ink-secondary border-b-2 border-line">
              ...
              <th className="px-3 py-2 text-left font-semibold">Alert Tier</th>
          ...
          <tbody>
            {pageRows.map((e, i) => (
              <tr key={`${e.client_id}-${e.sent_at}-${i}`} className="border-b border-line text-sm text-ink-primary hover:bg-surface-page transition-colors">
                ...
                <td className="px-3 py-2">{e.cert_name}</td>
                <td className="px-3 py-2">
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border border-line">
                    <span className={`h-1.5 w-1.5 rounded-full ${TIER_DOT[e.status_tier] || "bg-ink-muted"}`} aria-hidden="true" />
                    {e.status_tier}
                  </span>
                </td>
