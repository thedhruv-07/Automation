...
describe("importRenewals", () => {
  ...
  it("POSTs the files with apply=false for a preview", async () => {
  ...
  it("sends every file when several are chosen", async () => {
    global.fetch.mockResolvedValue({ ok: true, json: async () => ({}) });
    const second = new File(["y"], "haryana.xlsx");
    await importRenewals([file, second], false);
    expect(global.fetch.mock.calls[0][1].body.getAll("files")).toEqual([file, second]);
  });
